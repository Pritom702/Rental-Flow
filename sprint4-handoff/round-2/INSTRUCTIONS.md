# Sprint 4, round 2 — chunks for my groupmates

This is the second round of Sprint 4 — the polish-and-ship pass: **Bangladeshi Taka currency** across the
...
My part (M2 · @pritom702) is already pushed: branch **`s4-m2-platform`**,
...
|-------|-------|--------|----------|
| `chunk1-M1-charts.zip` | **M1 · @shaafin01** | `s4-m1-charts` | Chart palette tokens, Taka in analytics, item form label |
| `chunk3-M3-money-docs.zip` | **M3 · Promit** | `s4-m3-money-docs` | Taka in PDFs and statements, Taka-scaled seed prices |
| `chunk4-M4-design-profile.zip` | **M4 · Radowan** | `s4-m4-design-profile` | Design system, app shell, **F21 NID**, **F22 profile + payments**, **F23 renter check** |
| `chunk5-SHARED-bookings.zip` | **see note below** | `s4-shared-bookings` | The four co-authored files |
...
3. In VS Code open the Source Control panel, review the changed files, then:
   ...
   git push -u origin s4-<your-branch>
   ```
...
Unlike the first round of Sprint 4, these chunks are **not fully file-disjoint**,
because this round touched shared plumbing. Merge in this order to avoid conflicts and a broken
`main`:

```
1. s4-m2-platform          (mine — already pushed; money.js + app.js are imported by the rest)
2. s4-m4-design-profile    (styles.css, App.jsx routing, and the profile API the others reference)
3. s4-m1-charts            (uses the chart tokens added in step 2)
4. s4-m3-money-docs        (uses money.js from step 1)
5. s4-shared-bookings      (touches files the others also edit — merge last)
```
...
This round adds a schema file. After applying any chunk:
